...
> nằm ở [CHANGELOG-prototype.md](CHANGELOG-prototype.md); kiến trúc code ở `README.md`; spec kỹ thuật
...
| --- | --- | --- |
| Game Designer | 1, 2, 3, 8, 9, 10, 16 | 4, 5, 7 |
| Level Designer | 5, 6, 7, 8 | 3, 9 |
| Game Artist | 2, 11, 12 | 5 |
| UI/UX Designer | 2, 13 | 9, 10, 11 |
| Coder / Engineer | 3, 4, 5, 6, 14, 15 | mọi mục |
...
| 3 | **Mechanic là data** | Wall/Lock&Key/Freeze/Wind là field tuỳ chọn trong `SandLevelConfig`. Không khai báo field = level chạy y hệt bản không có mechanic. Solver không rẽ nhánh theo level. |
...
| Wind | field `wind` (không phải ký tự tranh) | — | Cài xong, **chưa level nào ship dùng** — chỉ trong fixture test |
...
Chuỗi pha gió (`direction`/`durationMs`/`cooldownMs`/`power`/`zone`) thổi cát lúc settle. Cài + test
đầy đủ ([wind.md](docs/features/wind.md)) nhưng chưa có trong `BUILT_IN_LEVELS` nào — dự trữ cho arc
tương lai.
...
| --- | --- | --- | --- | --- | --- |
| 0. Nhập môn | 1–3 | Ngắm-bắn (L1, 1 màu, `∞`) → hàng đợi đạn (L2, 3 màu, `∞`) → ngân sách lượt (L3) | 26.0* | 26 | 2.3 |
| 1. Beatchart gốc | 4–10 | Tranh nhiều màu/vùng, chưa mechanic phụ | 39.3 | 30–45 | 13.3 |
| 2. Wall Obstacle | 11–20 | Tính đường bắn quanh chướng ngại cứng | 42.5 | 30–55 | 10.6 |
| 3. Lock & Key | 21–30 | Trình tự mở khoá, đỉnh L29 (3 ổ khoá xếp tầng) | 49.0 | 35–65 | 13.0 |
| 4. Freeze Map | 31–40 | Chọn thời điểm bắn trigger đóng băng | 45.2 | 30–65 | 12.1 |
| 5. Tổng hợp | 41–50 | Trộn Wall + Lock & Key + Freeze cùng tranh | 56.5 | 40–80 | 13.0 |
...
- **Wind** (5.4) — engine xong, chưa có level dùng.
...
- Arc thứ 6 dùng Wind, hoặc trộn vào arc tổng hợp hiện có (level 51+).
